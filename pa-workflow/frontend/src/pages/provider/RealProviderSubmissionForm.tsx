...
const RealProviderSubmissionForm: React.FC = () => {
  ...
  const removeIcdCode = (code: string) => {
    setValue('icd10Codes', icd10Codes.filter((currentCode) => currentCode !== code), { shouldValidate: true })
  }
  ...
  const removeCptCode = (code: string) => {
    setValue('cptCodes', cptCodes.filter((currentCode) => currentCode !== code), { shouldValidate: true })
  }
  ...
  const handleFiles = useCallback(
    (files: File[] | FileList | null) => {
      ...
      if (validFiles.length > 0) {
        setValue('documents', [...documents, ...validFiles], { shouldValidate: true })
      }
    },
  ...
  const removeFile = (index: number) => {
    const nextFiles = documents.filter((_, currentIndex) => currentIndex !== index)
    setValue('documents', nextFiles, { shouldValidate: true })
  }
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      showNotification({
        ...
        message: `Your prior authorization request ${paId || 'has'} been submitted.`,
      })

      if (paId) {
        navigate(`/real-provider/status/${paId}`)
      }
    } catch (error) {
      showNotification({
        type: 'error',
    ...
    <div className="space-y-6 relative z-10 bg-white">
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2.5 bg-white border border-neutral-200 rounded-lg text-sm text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-[3px] focus:ring-primary-500/25 focus:border-primary-500 hover:border-neutral-300 transition-all duration-150"
          />
        ...
        render={({ field }) => (
          <div>
            ...
            <textarea
              {...field}
              rows={4}
              className="w-full px-3 py-2.5 bg-white border border-neutral-200 rounded-lg text-sm text-neutral-900 placeholder:text-neutral-400 focus:outline-none focus:ring-[3px] focus:ring-primary-500/25 focus:border-primary-500 hover:border-neutral-300 transition-all duration-150 resize-none"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Controller
          ...
          control={control}
          render={({ field }) => <Input {...field} label="Medication Dosage" placeholder="e.g., 40mg every 2 weeks" />}
        />
      </div>
    </div>
  )
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="flex justify-between mt-8 pt-6 border-t">
            ...
            {currentStep < 3 ? (
              ...
              <Button type="submit" loading={isSubmitting || submitPAMutation.isPending} disabled={documents.length === 0}>
                Submit PA Request
              </Button>
            )}
          </div>
